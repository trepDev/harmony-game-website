## A propos d'<span class="text-accent-2 dark:text-accent">Harmony Game</span>

### Tests pédagogiques et développement

Le système de graphe appliqué à l'harmonie musicale, ainsi que la cartographie des accords par couleur, ont été testés avec succès en université. Ces expériences pédagogiques ont été effectuées auprès d'étudiant en France et Pologne via des applications web interactives.  
La 1ère application mobile intégrant cette méthode d'apprentissage a été développé par Anna en 2016. Présentée au concours ELLESgorithme, elle a remporté le 2ème prix (le 1er prix n'a pas été attribué) de la meilleure application mobile d'éducation.  
La version actuelle est développée de toutes pièces par TrepDev et présente des fonctionnalités similaires à la version 2016 avec des améliorations graphiques et deux langues supplémentaires.


## A propos de l'équipe

### <span class="text-accent-2 dark:text-accent">Anna Shvets</span>: Compositrice

Anna Shvets est une compositrice de musique classique, gagnante de plusieurs concours de compositions. Elle possède un Master en composition ainsi qu'un doctorat en musicologie.  
Auteure d'une trentaine de publications scientifiques, elle a participé à de prestigieuses conférences scientifiques internationnales (Washington, Londres,Paris, Copenhagen ainsi que d'autres villes en Europe, Etats-unis et Asie).  
Plus d'informations à propos d'Anna :


- <a href="https://snd.click/l04w" target="_blank">Son dernier album musical</a>
- <a href="https://annashvetscomposer.wordpress.com/" target="_blank">Son blog personnel</a>
- <a href="https://www.researchgate.net/profile/Anna-Shvets-5" target="_blank">Ses publications scientifiques</a>

### <span class="text-accent-2 dark:text-accent">TrepDev</span>: Developpeur

Développeur Full Stack travaillant principalement dans les ecosystèmes Java, Angular & Flutter.  
N'hésitez pas à consulter son <a href="https://github.com/trepDev" target="_blank">profil GitHub</a>.

## Un grand merci pour le thème graphique

### <span class="text-accent-2 dark:text-accent">J.C Henry</span>: Motion Designer / Art Director

N'hésitez pas à consulter son <a href="https://jiss.tv" target="_blank">site personnel</a>.

### <span class="text-accent-2 dark:text-accent">Clémentine Laroche</span>: Motion Designer / Art Director

N'hésitez pas à consulter son <a href="http://www.clementinelaroche.com/" target="_blank">site personnel</a>.

